...
def question_exists(self):
    try:
        judge_util.extract_special_unique_cell(self.submission, judge_util.QUESTION_CELL_NAME)
    except KeyError:
        judge_util.set_fail_tag(self, judge_util.EvaluationTag('QCM', 'Question Cell Missing', '#333333', '#cccccc'))
        self.fail()
...
def answer_exists(self):
    try:
        judge_util.extract_special_unique_cell(self.submission, judge_util.ANSWER_CELL_NAME)
    except KeyError:
        judge_util.set_fail_tag(self, judge_util.EvaluationTag('ACM', 'Answer Cell Missing', '#333333', '#cccccc'))
        self.fail()
